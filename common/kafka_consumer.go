...
func (kc *KafkaConsumer) runSandbox() {
	fmt.Println("runSandbox")
	atomic.AddInt32(&kc.idleNum, 1)
	defer func() {
		atomic.AddInt32(&kc.idleNum, -1)
	...
	logs.Log(logs.F{"brokes": kc.brokers, "group": kc.group, "topic": kc.topics})
	...
	for {
		...
		case <-kc.stopCh:
			...
			fmt.Println("kafka worker close")
			kc.stopFlag = true
			kc.stopCh <- struct{}{}
			return
		}
...
func (kc *KafkaConsumer) Close() {
	...
	<-kc.stopCh
	for {
		if atomic.LoadInt32(&kc.idleNum) == 0 {
			break
		}
		fmt.Println("aysnc close", kc.idleNum, kc.stopFlag)
